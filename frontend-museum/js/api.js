...
async function downloadPDF(id, nomorSurat, tampilkanKondisi = true) {
  try {
    const qs  = tampilkanKondisi ? '' : '?tampilkan_kondisi=0';
    const res = await fetch(`${BASE_URL}/berita-acara/${id}/pdf${qs}`, {
      headers: { 
        'Authorization': `Bearer ${getToken()}`,
        'ngrok-skip-browser-warning': 'true' 
      }
...
const API = {
  ...
  updateStatusKembali:  (id)     => apiFetch(`/berita-acara/${id}/status-kembali`, { method: 'PUT' }),
